...
const AdminVideos = () => {
  ...
  const [selectedLessonId, setSelectedLessonId] = useState<string>(searchParams.get('lesson') || 'all');
  const [selectedTopicId, setSelectedTopicId] = useState<string>(searchParams.get('topic') || '');
  ...
  useEffect(() => {
    fetchLessons();
    // If topic param exists, fetch videos by topic
  ...
  useEffect(() => {
    // Only fetch videos when lesson is selected (not on initial mount - that's handled in first useEffect)
  ...
  useEffect(() => {
    if (selectedTopicId && selectedLessonId && selectedLessonId !== 'all') {
  ...
  useEffect(() => {
    ...
    // If lesson param exists in URL, use it
    if (lessonParam && lessonParam !== 'all') {
      setSelectedLessonId(lessonParam);
      fetchTopicsByLesson(lessonParam);
      // If topic param exists, set it
      if (topicParam) {
        setSelectedTopicId(topicParam);
      }
    } else if (selectedLessonId && selectedLessonId !== 'all') {
      fetchTopicsByLesson(selectedLessonId);
    } else {
      setTopics([]);
      setSelectedTopicId(''); // Reset topic filter when lesson is cleared
    }
  }, [selectedLessonId, searchParams]);
  ...
  const fetchLessons = async () => {
    try {
      ...
      // Don't auto-select first lesson - let user choose or show all videos by default
    } catch (err: any) {
  ...
  const fetchVideosByTopic = async (topicId: string) => {
    try {
      ...
      console.error('❌ Error fetching videos by topic:', err);
      setError(err.response?.data?.message || err.message || 'Failed to fetch videos by topic');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, videoTitle: string) => {
    ...
    try {
      ...
      // Refresh videos based on current selection
      if (selectedTopicId && selectedLessonId && selectedLessonId !== 'all') {
        fetchVideosByTopic(selectedTopicId);
      } else if (selectedLessonId) {
        fetchVideosByLesson(selectedLessonId);
  ...
  const filteredVideos = videos.filter(video => {
    ...
    const matchesTopic = !selectedTopicId || video.topic?._id === selectedTopicId;
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
            ...
            <div className="flex gap-4">
              ...
              <select
                ...
                disabled={selectedLessonId === 'all' || !selectedLessonId}
                className="w-full md:w-1/2 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
              >
